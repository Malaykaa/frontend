...
import{approveAdminStructure,deleteAdminOffer,deleteAdminStructure,deleteAdminThread,deleteAdminUser,fetchAdminDocuments,fetchAdminGoals,fetchAdminIntents,fetchAdminOffer,fetchAdminOffers,fetchAdminStats,fetchAdminStructure,fetchAdminStructures,fetchAdminThread,fetchAdminThreads,fetchAdminUser,fetchAdminUsers,fetchScrapingStats,rejectAdminStructure,runApifyScraping,runPerplexityScraping,updateAdminOffer,updateAdminStructure,updateAdminUser}from"@/services/api/admin.api";
import type{AdminDocumentsParams,AdminGoalsParams,AdminIntentsParams,AdminOffersParams,AdminStructuresParams,AdminThreadsParams,AdminUsersParams}from"@/services/api/admin.api";
import type{AdminOfferUpdate,AdminUserUpdate}from"@/shared/types";
export const adminKeys={stats:["admin","stats"]as const,users:(p:AdminUsersParams)=>["admin","users",p]as const,user:(id:string)=>["admin","users",id]as const,offers:(p:AdminOffersParams)=>["admin","offers",p]as const,goals:(p:AdminGoalsParams)=>["admin","goals",p]as const,threads:(p:AdminThreadsParams)=>["admin","threads",p]as const,thread:(id:string)=>["admin","threads",id]as const,docs:(p:AdminDocumentsParams)=>["admin","documents",p]as const,intents:(p:AdminIntentsParams)=>["admin","intents",p]as const,scrapingStats:["admin","scraping","stats"]as const,structures:(p:AdminStructuresParams)=>["admin","structures",p]as const,structure:(id:string)=>["admin","structures",id]as const};
export const useAdminStructures=(p:AdminStructuresParams={})=>useQuery({queryKey:adminKeys.structures(p),queryFn:()=>fetchAdminStructures(p),staleTime:30000});
export const useAdminStructure=(id:string)=>useQuery({queryKey:adminKeys.structure(id),queryFn:()=>fetchAdminStructure(id),enabled:!!id,staleTime:0});
export function useApproveAdminStructure(){const qc=useQueryClient();return useMutation({mutationFn:(id:string)=>approveAdminStructure(id),onSuccess:(_,id)=>{qc.invalidateQueries({queryKey:["admin","structures"]});qc.invalidateQueries({queryKey:adminKeys.structure(id)});toast.success("Structure approuvée.")},onError:(err:unknown)=>toast.error(err instanceof Error?err.message:"Erreur.")})}
export function useRejectAdminStructure(){const qc=useQueryClient();return useMutation({mutationFn:(id:string)=>rejectAdminStructure(id),onSuccess:(_,id)=>{qc.invalidateQueries({queryKey:["admin","structures"]});qc.invalidateQueries({queryKey:adminKeys.structure(id)});toast.success("Structure refusée.")},onError:(err:unknown)=>toast.error(err instanceof Error?err.message:"Erreur.")})}
export function useUpdateAdminStructure(){const qc=useQueryClient();return useMutation({mutationFn:({id,payload}:{id:string;payload:{status?:string}})=>updateAdminStructure(id,payload),onSuccess:(_,{id})=>{qc.invalidateQueries({queryKey:["admin","structures"]});qc.invalidateQueries({queryKey:adminKeys.structure(id)});toast.success("Structure mise à jour.")},onError:(err:unknown)=>toast.error(err instanceof Error?err.message:"Erreur.")})}
export function useDeleteAdminStructure(){const qc=useQueryClient();return useMutation({mutationFn:(id:string)=>deleteAdminStructure(id),onSuccess:()=>{qc.invalidateQueries({queryKey:["admin","structures"]});toast.success("Structure supprimée.")},onError:(err:unknown)=>toast.error(err instanceof Error?err.message:"Erreur.")})}
...
export function useUpdateAdminUser(){const qc=useQueryClient();return useMutation({mutationFn:({userId,payload}:{userId:string;payload:AdminUserUpdate})=>updateAdminUser(userId,payload),onSuccess:(_,{userId})=>{qc.invalidateQueries({queryKey:["admin","users"]});qc.invalidateQueries({queryKey:adminKeys.user(userId)});toast.success("Utilisateur mis à jour.")},onError:(err:unknown)=>toast.error(err instanceof Error?err.message:"Erreur.")})}
export function useDeleteAdminUser(){const qc=useQueryClient();return useMutation({mutationFn:(id:string)=>deleteAdminUser(id),onSuccess:()=>{qc.invalidateQueries({queryKey:["admin","users"]});qc.invalidateQueries({queryKey:adminKeys.stats});toast.success("Utilisateur supprimé.")},onError:(err:unknown)=>toast.error(err instanceof Error?err.message:"Erreur.")})}
export function useUpdateAdminOffer(){const qc=useQueryClient();return useMutation({mutationFn:({offerId,payload}:{offerId:string;payload:AdminOfferUpdate})=>updateAdminOffer(offerId,payload),onSuccess:()=>{qc.invalidateQueries({queryKey:["admin","offers"]});toast.success("Offre mise à jour.")},onError:(err:unknown)=>toast.error(err instanceof Error?err.message:"Erreur.")})}
export function useDeleteAdminOffer(){const qc=useQueryClient();return useMutation({mutationFn:(id:string)=>deleteAdminOffer(id),onSuccess:()=>{qc.invalidateQueries({queryKey:["admin","offers"]});qc.invalidateQueries({queryKey:adminKeys.stats});toast.success("Offre supprimée.")},onError:(err:unknown)=>toast.error(err instanceof Error?err.message:"Erreur.")})}
export function useDeleteAdminThread(){const qc=useQueryClient();return useMutation({mutationFn:(id:string)=>deleteAdminThread(id),onSuccess:()=>{qc.invalidateQueries({queryKey:["admin","threads"]});qc.invalidateQueries({queryKey:adminKeys.stats});toast.success("Thread supprimé.")},onError:(err:unknown)=>toast.error(err instanceof Error?err.message:"Erreur.")})}
