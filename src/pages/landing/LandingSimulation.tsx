...
import {
  Briefcase, MapPin, GraduationCap, Banknote, FileCheck, type LucideIcon,
} from "lucide-react";
import { AiAvatar } from "@/components/chat/AiAvatar";
import { AnimatedDots } from "@/components/chat/AnimatedDots";

/**
 * Simulation d'accueil — montre Malayka au lieu de la décrire.
 *
 * Reprend le langage visuel exact du produit (bulles, AiAvatar, AnimatedDots),
 * comme ObjectiveSimulation dont elle dérive, avec une différence qui est tout
 * l'intérêt de cette version : une quatrième phase où l'offre arrive sur
 * WhatsApp. C'est la promesse la plus concrète de Malayka — la veille continue
 * même quand personne n'a l'application ouverte — et c'est très difficile à
 * faire comprendre par un texte.
 *
 * Note sur le vert : notre identité est monochrome, et cette carte y déroge
 * volontairement. Le vert n'est pas notre accent, c'est la signature d'une
 * marque tierce : c'est précisément ce qui fait comprendre en un coup d'œil
 * que le message arrive sur WhatsApp, sans avoir à l'écrire.
 */
...
interface Scenario {
  ...
  /** Ce que Malayka pousse ensuite sur WhatsApp, sans que l'app soit ouverte. */
  whatsapp: string;
}
...
// Rythme : chaque phase laisse le temps de lire avant la suivante. La phase
// WhatsApp reste affichée plus longtemps — c'est celle qu'on veut retenir.
...
export function LandingSimulation() {
  ...
  return (
    // Hauteur minimale fixe : sans elle, la mise en page sauterait à chaque
    // changement de scénario, les textes n'ayant pas la même longueur.
    <div className="mx-auto w-full max-w-sm space-y-3 rounded-2xl border bg-card p-4 shadow-sm min-h-[340px]">
      {stage >= 1 && (
